...
def is_binary_file(p: Path, sample_size: int = 8192) -> bool:
    try:
        with p.open("rb") as f:
...
def build_tree(root: Path, ignore_ids: set[str] | None = None) -> str:
...
def build_contents(root: Path, files: Iterable[Path], ns: bool = False) -> str:
    parts: list[str] = []
...
def main() -> None:
    parser = argparse.ArgumentParser(prog="ayn", description="Сохранение структуры проекта и содержимого файлов")
    ...
    if args.cmd == "struc":
        ...
        tree_text = build_tree(root, ignore_ids={path_id(out)})
        out.write_text(tree_text + "\n", encoding="utf-8")
        print(str(out))
        return

    if args.cmd == "cont":
        if args.ns and (args.paths or args.exclude):
            parser.error("-ns нельзя использовать вместе с позиционными путями или -ex")
        ...
        if args.ns:
            ...
            excluded = expand_targets(root, args.exclude)
            excluded_ids = {path_id(p) for p in excluded}
            files = [p for p in all_files(root) if path_id(p) not in excluded_ids]
        elif args.paths:
            files = expand_targets(root, args.paths)
        else:
            files = all_files(root)

        tree_text = build_tree(root, ignore_ids=ignore_ids)
        content_text = build_contents(root, files, ns=args.ns)
